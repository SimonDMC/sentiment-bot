...
client.on(Events.MessageUpdate, async (oldMessage, newMessage) => {
    // take from sentiment channel or through dm
    if (newMessage.channel.name != "sentiment" && newMessage.guild != null)
        return;

    // remove reactions
    const reactions = newMessage.reactions.cache;
    reactions.forEach((reaction) => reaction.users.remove(client.user.id));

    rateMessage(newMessage);
});

async function rateMessage(message) {
    try {
